...
/* This function removes a specific task by its index */
function remove() {
    var id = this.getAttribute('id'); // Get the id attribute of the clicked button
    var todos = get_todos();
    todos.splice(id, 1); // Removes 1 item at the given index
    localStorage.setItem('todo', JSON.stringify(todos)); // Updates the localStorage
    show(); // Refreshes the displayed list
    return false;
}
...
function show() {
    ...
    /* Attach event listeners to "x" buttons */

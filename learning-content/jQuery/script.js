$(document).ready(function(){ //This function will run when the document is finished loading.
    ...
    $(document).on("scroll", function() {
        if ($(document).scrollTop() > 50) {
            $("h1").addClass("header-scroll");
        } else {
            $("h1").removeClass("header-scroll");
        }
    });

    //Triigers the images to slide down one after the other when the user's mouse
    ...
    //Creates a toggle efffect on each faq question by checking each element's sibling so they don't
    //all display when one question is clicked
